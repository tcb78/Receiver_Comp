...
public class MainActivity extends Activity implements OnCheckedChangeListener {
            ...
            fft = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    while (isRecording) {
                        ...
                        int targetDecibelAve = computeComparisonTargetDecibelAverage(RECEIVING_FREQ, decibelFrequencySpectrum);
                        //ドップラー効果を考慮した接近検知
                        detectApproaching(RECEIVING_FREQ, targetDecibelAve + DECIBEL_DIFF, decibelFrequencySpectrum);
                        //計測終了
                        tm.finishMeasure();
                        //処理時間出力
                        tm.printTimeSec();
     ...
     */
    public int computeComparisonTargetDecibelAverage(int freq, double[] decibelFrequencySpectrum) {
        //比較対象の周波数を設定 comparison target
        int targetDecibelAve = 0, freqFrameIter = 0;
        int targetFrameBegin = frameSetting(freq + 500);
        int targetFrameEnd = frameSetting(freq + 700);

        for (int j = targetFrameBegin; j < targetFrameEnd; j++) {
    ...
    public void detectApproaching(int freq, double decibel, double[] decibelFrequencySpectrum) {
        ...
        for(int i = detectFrameBegin; i < detectFrameEnd; i++) {
            if(decibelFrequencySpectrum[i] > decibel) {
                vib.cancel();
                vib.vibrate(200);
                break;
            }
